...
Meet http://gpars.org[*GPars*], an open-source concurrency and parallelism library for *Java* and *Groovy* that gives you a number of high-level abstractions to write concurrent and parallel code in *Groovy* 
...
Please refer to our link:./guide[most recent User Guide] or any of our older link:User_Guide.html[User Guides] for an extensive coverage of *GPars* abstractions. You may also like a few link:Demos.html[Demos] to get a taste of what's in here for you.
